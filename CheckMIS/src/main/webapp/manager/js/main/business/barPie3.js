...
var myChart = echarts.init($("#insurbar")[0], "shine"); //初始化地图 
option = {
  /*title : {
      text: '服务效率',
			x: 'center',
...
var myChart2 = echarts.init($("#insurpie")[0], "shine"); //初始化地图 
option2 = {
  title : {
      text: '分期区域占比',
			x: 'right',
...
comn.ajax({
   url: interUrl.dataView.insuranceGetAll,
...
getByStype('1');
$('select[name=insurCardTime]').change(function(){
    var value=$(this).val();
    getByStype(value);
});
function getByStype(stype){
    comn.ajax({
       url: interUrl.dataView.insuranceGetByStype,
       data: {stype: stype},
       success: function (res) {
        if(res.data){
