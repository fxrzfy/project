...
var feeArr=["insuranceAmount","accountDeposit","renewalDeposit","pbDeposit","costAmount","purchaseTax","guarantyRiskAmount","serviceFee","gpsInstallationFee","liabilityAmount","agencyFee1","agencyFee2","otherFee","bankIrsFee","licenseFee","doorSurveyFee","agencyFee","outsourceSurveyFee", "valuationFee","xzzDiscountAmount","dealerPayBackAmount"];
//计算费用合计
function countTotalFee(){
    var feeArr3=["insuranceAmount","accountDeposit","renewalDeposit","pbDeposit","costAmount","purchaseTax","guarantyRiskAmount","serviceFee","gpsInstallationFee","liabilityAmount","bankIrsFee","licenseFee","doorSurveyFee","agencyFee","outsourceSurveyFee", "valuationFee","dealerPayBackAmount"];
    var totalFee = 0;
...
function counttestedRepaymentAmount(){  //试算月还款额=【贷款金额*(1+手续费率)-车商贴息政策】/贷款期限(月)    业务品种=4时,试算月还款额=贷款金额*(1+公司手续费率)/期限(月)
    ...
    if(!isNaN($("#loanAmount").val())&&!isNaN($("[name='handingFee']").val())){
    	if(jiarongDisposeWay == 1){//v1.1.0补丁版本，新增将加融费用添加到贷款额的计算方式，此方式的试算月还款额的计算方法为
    		var jrf=parseFloat(jiaRongFee);
    		var la=parseFloat($("#loanAmount").val())|| "0";
    		var bhf=parseFloat($("#baseHandingFee").val())|| "0";
    		fee=Math.ceil(comn.Div(comn.Mul(comn.Add(jrf,la),comn.Add(100,bhf)),comn.Mul(100,qs)));
...
$("#housingStatus").change(function(){
    ...
    if(_this.val()==1 || _this.val()==""){
        ...
        c.show().find("label").eq(0).html(a+"月还款:");
    }else if(_this.val()==3){
        c.show().find("label").eq(0).html(a+"月租:");
    }else if(_this.val()==4){
        c.show().find("label").eq(0).html(a+"说明:");
    }
});
//是否贴息
...
//			}
			var accommodateTypeName = getAccommodateTypeNameByValue(res.data.accommodateType);
            $("#accommodateTypePrompt").append(accommodateTypeName); //通融单显示
            if (res.data.isAccomodate == 1 && accommodateTypeName != undefined && accommodateTypeName != "") {
                $("#accommodateTypePrompt").removeClass("hide");
            } else {
                $("#accommodateTypePrompt").addClass("hide");
            }
          ...
          //是否车商试单
        	if(res.data.signStatus==0){
        		var a='<span class="isShowImg" id="dealerTry">车商试单中</span>';
        		$(".flow-isShowImg").append(a)
        	}
            ...
            if(housingStatus!="" || housingStatus!=null){
                if(housingStatus==2){//贷款购房（住房状况若选择“贷款购房” 和 “租房”，增加文案提示）
                	var span = '<span class="text-danger" id="spanV">需在 “影像管理 - 资产证明” 中上传房产证照片</span>';
            		$("#dsWangTi").children(":last").parent().append(span);
            		
                    c.show().find("label").html(a+"月还款:");
                }else if(housingStatus==3){//租房（住房状况若选择“贷款购房” 和 “租房”，增加文案提示）
                	var span = '<span class="text-danger" id="spanV">需在 “影像管理 - 资产证明” 中上传租房协议照片</span>';
            		$("#dsWangTi").children(":last").parent().append(span);
            ...
            if((args["releventFlowNode"] == "LOAN_OFFICE_STAFF_BUDGET")||(args["releventFlowNode"] == "LOAN_MODIFY_OFFICE_STAFF_BUDGET")){
            	if(res.data.cardType != "" && res.data.cardType != undefined && res.data.cardType != null){
            	...
            	$('#birthday').change(function(){
            		$("#age").val(getAgeBybirth($("#birthday").val()));
            	});
            }
        }
    });
    ...
    $("#getCompanyAddressPosOffice").click(function () {
        province = $("#province_2_name").val();
        city = $("#city_2_name").val();
        area = $("#area_2_name").val();
        setTimeout(function(){
            $("#mapSelfModal").modal("show");
            map = new BMap.Map("mapCommon");
            menu = new BMap.ContextMenu();
            map.addContextMenu(menu);
            map.centerAndZoom("杭州", 12);
            if(city == "县"){
            	setPlaceInMap(province + area + $("#companyAddressDetail").val(),'获取当前位置','#companyPosition','#companyLongitude','#companyLatitude');
            }else{
            	setPlaceInMap(province + city + area + $("#companyAddressDetail").val(),'获取当前位置','#companyPosition','#companyLongitude','#companyLatitude');
            }
        },500);
    });

    $("#getHomeAddressPosOffice").click(function () {
        province = $("#province_1_name").val();
        city = $("#city_1_name").val();
        area = $("#area_1_name").val();
        setTimeout(function(){
            $("#mapSelfModal").modal("show");
            map = new BMap.Map("mapCommon");
            menu = new BMap.ContextMenu();
            ...
            map.centerAndZoom("杭州", 12);
            if(city == "县"){
            	setPlaceInMap(province + area + $("#homeAddressDetail").val(),'获取当前位置','#homePosition','#homeLongitude','#homeLatitude');
            }else{
            	setPlaceInMap(province + city + area + $("#homeAddressDetail").val(),'获取当前位置','#homePosition','#homeLongitude','#homeLatitude');
            }
        },500);
    });
    
    $("#getSpouseCompanyAddressPosOffice").click(function () {
        province = $("#province_3_name").val();
        city = $("#city_3_name").val();
        area = $("#area_3_name").val();
        setTimeout(function(){
            $("#mapSelfModal").modal("show");
            map = new BMap.Map("mapCommon");
            menu = new BMap.ContextMenu();
            ...
            map.centerAndZoom("杭州", 12);
            if(city == "县"){
            	setPlaceInMap(province + area + $("#spouseCompanyAddress").val(),'获取当前位置','#spouseCompanyPosition','#spouseCompanyLongitude','#spouseCompanyLatitude');
            }else{
            	setPlaceInMap(province + city + area + $("#spouseCompanyAddress").val(),'获取当前位置','#spouseCompanyPosition','#spouseCompanyLongitude','#spouseCompanyLatitude');
            }
        },500);
    });

    setPlaceInMap = function (address,title,myPosition,myLongitude,myLatitude) {//设置地图地点坐标
        var local, sc;
        map.clearOverlays();
...
$(document).on("blur", "#loanAmount", function(){
	...
	var isLoanJiaRongOr = $('input[name="isLoanJiaRongOr"]:checked').val();
	comn.ajax({
	    url: interUrl.loanDetail.getMinBillingPrice,
	    data: $.extend(budgetInfoVal, loanApplyId, {jiaRongPlanId:jiaRongPlanId}, {isLoanJiaRongOr:isLoanJiaRongOr}),
	    success: function (res) {
	        $("input[name=minBillingPrice]").val(res.data);
	    }
	});
