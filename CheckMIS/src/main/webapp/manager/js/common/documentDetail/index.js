var args, base64, cureent_dir, freeObj, getDocumentList, getSelectImage, loadTree, treeObj, targetDir, treeTarget,
    zTreeOnClick, allImg, imgIds = {};
var dataid;
var treeTargetBank;
jQuery.browser = {};
...
if ("LOAN_DOCUMENT_CHECK" == args['releventFlowNode']) {
    $("#dataCheckRow").css("display", "block");
}
var zTreeObj;
var current;
(function () {
    ...
    imgListHtml = function (o, list) {
        ...
        for (j = 0, len = list.length; j < len; j++) {
            ...
            var flowNodeNameStr = "   ——   ";
            var createTimeStr = "      ——      ";
            if(item.userName != "" && item.userName != undefined && item.userName != null){
            	userNameStr = item.userName;
            }
            if(item.device != "" && item.device != undefined && item.device != null){
            	deviceStr = item.device;
            }
            if(item.flowNodeName != "" && item.flowNodeName != undefined && item.flowNodeName != null){
            	flowNodeNameStr = item.flowNodeName;
            }
            if(item.createTime != "" && item.createTime != undefined && item.createTime != null){
            ...
            results.push([
                ...
                "<div>" +
                "<div style='width: 80%;float: left;text-overflow: ellipsis;overflow: hidden;white-space: nowrap;' title='" + item.fileName + "'>" + item.fileName + "</div>" +
                "<div style='width: 20%;float: right;'><i class='glyphicon glyphicon-eye-open' style='float:right; color: " + (item.hasRead === 1 ? "#CCD5D3" : item.objectType == "loan_patch" ? "red" : "#1ab394") + "'></i> </div> " +
                "</div>",
                "<div style='text-overflow: ellipsis;overflow: hidden;white-space: nowrap; clear: both;'>" + userNameStr + " | " + deviceStr + " | " + flowNodeNameStr + "</div>",
                item.imei == "" || item.imei == undefined || item.imei == null ? "" : "<div style='text-overflow: ellipsis; overflow: hidden; white-space: nowrap;' >IMEI：" + item.imei + "</div>",
                "<div>" +
                "<div style='float:left;width:80%;text-overflow: ellipsis; overflow: hidden; white-space: nowrap;' > 上传时间：" + createTimeStr + "</div>" +
                (cureent_dir.canDelete === 1 || cureent_dir.canMove === 1 ? "<div style='float: right;width:20%;'><input type='checkbox' name='pic' class='pull-right' value='" + item.id + "' style='margin: 0; margin-top: 2px;' /></div>" : "<div style='float: right;width:20%;'><input type='checkbox' name='pic' class='pull-right' value='" + item.id + "' style='margin: 0; margin-top: 2px;' /></div>") +
                "</div>",
                item.imei == "" || item.imei == undefined || item.imei == null ? "<div style='text-overflow: ellipsis;overflow: hidden;white-space: nowrap; clear: both;'>&nbsp;</div>" : "",

                // "<p style='text-overflow: ellipsis; overflow: hidden; white-space: nowrap;' title='"+ item.fileName +"'>" + item.fileName +
                // "<i class='glyphicon glyphicon-eye-open' style='float:right; color: " + (item.hasRead === 1 ?"#CCD5D3": item.objectType == "loan_patch"? "red": "#1ab394") + ";'></i>" + "</p>",
                // "<p style='text-overflow: ellipsis; overflow: hidden; white-space: nowrap;' >" + item.userName + " | " + item.device + " | " + item.flowNodeName +"</p>",
                // item.imei == "" || item.imei == undefined || item.imei == null ? "" : "<p style='text-overflow: ellipsis; overflow: hidden; white-space: nowrap;' >" + item.imei +"</p>",
                // "<p style='text-overflow: ellipsis; overflow: hidden; white-space: nowrap;' > 上传时间：" +
                // item.createTime + (cureent_dir.canDelete === 1 || cureent_dir.canMove === 1 ? "<input type='checkbox' name='pic' class='pull-right' value='" + item.id + "' style='margin: 0; margin-top: 2px;' />" : "<input type='checkbox' name='pic' class='pull-right' value='" + item.id + "' style='margin: 0; margin-top: 2px;' />")+
                // "</p>",
                //  item.imei == "" || item.imei == undefined || item.imei == null ? "<p>&nbsp;</p>" : "",
                "</div>",
                "</div>",
                "</div>"
            ].join(""));
                ...
                if (args['loanFlag'] != '2') {
                    ...
                    comn.ajax({
                        ...
                        success: function (res) {
                            ...
                            if ((args['licenseFlowFlag'] == 'licenseFlowFlag') || (args['pledgeFlowFlag'] == 'pledgeFlowFlag')) {
                                $("input[name=documentCheckStatus]").attr("disabled", "disabled");
                                $("#subTree").hide();
                            }
                        }
                    });
                    comn.ajax({
                        url: "licensePlateInfoVo/getLicensePlateInfoByProjectId",
                        data: {
                            projectId: args['projectId']
                        },
                        success: function (res) {
                            var registerDate;
                            if (res.data.registerDate != "" && res.data.registerDate != undefined && res.data.registerDate != null) {
                                registerDate = res.data.registerDate.substring(0, 10);
                            }
                            $("#registerCheck").values({
                                registerDate: registerDate,
                                plateNo: res.data.plateNo,
                                registerTarget: res.data.registerTarget
                            })
                        }
                    });
                    comn.ajax({
                        url: "infoQuery/getPledgeInfoByProjectId",
                        data: {
                            projectId: args['projectId']
                        },
                        success: function (res) {
                            var pledgeDate;
                            if (res.data.pledgeDate != "" && res.data.pledgeDate != undefined && res.data.pledgeDate != null) {
                                pledgeDate = res.data.pledgeDate.substring(0, 10);
                            }
                            $("#pledgeCheck").values({
                        ...
                        zTreeObj = $.fn.zTree.init($("#bankTree"), {
                            ...
                            view: {
                                fontCss: function (treeId, treeNode) {
                                    return (treeNode.dirNum1 == 'divigeneral') ? {background: 'rgba(255,255,204,0.8)'} : '';
                                }
                            }
                        }, zNodes);
                        //移动目录
                        treeTargetBank = $.fn.zTree.init($("#treeTargetBank"), {
                            showLine: true,
                            expand: true,
                            callback: {
                                onClick: function (event, treeId, treeNod) {
                                    if (!treeNod.isParent) {
                                        targetDir = treeNod;
                                        return $("#targetSure2").removeClass("disabled");
                                    } else {
                                        return $("#targetSure2").addClass("disabled");
                                    }
                                }
                            }
                        }, zNodes);
                        treeTargetBank.expandAll(true);
                    }
                })
    ...
    handle2 = function (o) {
        return comn.ajax({
            ...
            data: {
                loanApplyId: m.loanApplyId,
                dirId: o.dirId,
                documentIds: getSelectImageBank()['id'].join(","),
                fileNamespace: args['space'] || "",
                releventFlow: args['releventFlow'] || "",
                releventFlowNode: args['releventFlowNode'] || ""
            },
...
$(function () {
    //如果是资料复核节点，显示银行文件夹
    if (args['releventFlowNode'] == "APPLYDIVIGENERAL") {
    ...
    $("#moveImage2").click(function () {
        if (!$(this).hasClass("disabled")) {
            return $("#treeTargetBankDir").modal("show");
        }
    });
    ...
    $("#upImage2").click(function () {
        if (!$(this).hasClass("disabled")) {
            return $("#upImageInput2").trigger("click");
        }
    });
    ...
    $("#targetSure2").click(function () {
        if (!$(this).hasClass("disabled")) {
            return handle2({
                url: interUrl.gr.moveDocument,
                dirId: targetDir['id'],
                callback: function (res) {
                    return $("#treeTargetBankDir").modal("hide");
                }
            });
        }
    });
        ...
        for (k = j = 0, len = fileArr.length; j < len; k = ++j) {
            ...
            results.push(base64(i, k, function (f, o, index) {
                html = ["<tr>",
                    ...
                    "</td>",
                    "</tr>"].join("");
                return $("#fileList2 tbody").prepend(html);
            }));
        }
        return results;
    });
    ...
    $("#fileList2 tbody").on("click", ".upCancle", function () {
        return $(this).parents("tr").remove();
    });
    $("#upload2").click(function () {
        var $tr, $trAll, maxImg, num, upImg;
        $tr = $("#fileList2 tbody").find("tr:not('.loaded')");
        $trAll = $("#fileList2 tbody").find("tr");
        num = $tr.index();
        if (num === -1) {
            return;
        }
