...
zTreeOnClickSub = function(event, treeId, treeNod) {
    ...
    $("#wtflId").val(current_sub_node.id);
...
table_1 = function (params) {
    tableData(params,$("#searchForm").values(), interUrl.dict.wentiwtxList);
};
    ...
    comn.ajax({
        ...
        success: function(res) {
            let childs=$(res.data).map(function (i,ele) {
                return {name:ele.wtms,id:parseInt(ele.id),pId:parseInt(ele.pid)}
            }).get();
...
saveOrg = function(_form, _callback){
    ...
    var _url = interUrl.dict.wentiaddWtFl
    return comn.ajax({
        url: _url,
        data: _data,
        success: function(res) {
...
saveDetail = function(_form, _callback){
    var _data = _form.values();
    var _url = interUrl.dict.wentiaddWtXD
    return comn.ajax({
        url: _url,
        data: _data,
        success: function(res) {
...
$("#delAll").click(function () {
    ...
    $("#sureOption").unbind("click").click(function (){
        comn.ajax({
            url: interUrl.dict.wentidelWtXD,
            data: {
                ids: arr.join(",")
            },
...
$(function() {
    $("#add").click(function() {
        $("#addOrg").modal("show");
        $("#addOrgForm input[name='type']").val(current_node.type);
        $("#addOrgForm input[name='pid']").val(current_sub_node.id);
    });
    $("#modify").click(function() {
        ...
        $("#addOrgForm input[name='pid']").val(current_sub_node.pId);
        $("#addOrgForm input[name='wtms']").val(current_sub_node.name);
    ...
    $("#del").click(function() {
        ...
        $("#sureOption").unbind("click").click(function (){
            comn.ajax({
                url: interUrl.dict.wentidelWtFl,
                data: {
    ...
    $("#addDetailNew").click(function() {
        $("#addDetail").modal("show");
        $("#addDetailForm input[name='wtflId']").val(current_sub_node.id);
    });
